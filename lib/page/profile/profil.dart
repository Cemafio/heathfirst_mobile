...
class ProfilSection extends ConsumerStatefulWidget {
  const ProfilSection({super.key});

  @override
  ConsumerState<ProfilSection> createState() => _ProfilSectionState();
}

class _ProfilSectionState extends ConsumerState<ProfilSection> {
  ...
  void initState() {
    ...
    // _countDemdType();
    // _infoUser = userInfo();
    _listDemd = rdvUserData(baseUrl: ref.watch(baseUrl), token: ref.watch(accessTokenProvider));
  }

  Future<void> _openEditProfil() async {
    final updated = await Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => (ref.read(userDataStatic).roles == 'ROLE_PATIENT')? Editprofil(): EditprofilDoc()),
    );
  ...
  Widget build(BuildContext context) {
    ...
    final asyncUserData = ref.watch(user_data);
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        child: asyncUserData.when(
          ...
          error: (e, st){
            return Text("Error de chargement des donner de l'user");
          },
          data: (user){
            return 
              Column(
                        ...
                        Row(
                          ...
                          children: [
                            if(user.roles =='ROLE_DOCTOR')...[
                              GestureDetector(
                                onTap: (){
                                  _navigation(AjouterCabinetPage());
                                },
                                child: Container(
                                    width: 50,
                                    height: 50,
